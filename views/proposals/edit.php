...
<p class="text-muted mb-4">
  <a href="<?= url("/proposals/{$proposal['id']}") ?>"><i class="bi bi-arrow-left me-1"></i>Back to proposal</a>
</p>
  ...
  <div class="col-xl-11">
    <form method="post" action="<?= url("/proposals/{$proposal['id']}/edit") ?>" id="proposalForm">
          ...
          <div class="row g-3">
              ...
              <div class="input-group">
                <select name="client_id" class="form-select" required>
                  ...
                  <?php foreach ($clients as $cl): ?>
                    <option value="<?= $cl['id'] ?>" <?= old('client_id', $proposal['client_id']) == $cl['id'] ? 'selected' : '' ?>>
                      <?= e($cl['name']) ?>
                    </option>
                  <?php endforeach ?>
                </select>
            ...
            <div class="col-md-3">
              ...
              <input type="text" name="project_name" class="form-control"
                     value="<?= old('project_name', e($proposal['project_name'])) ?>" required
                     placeholder="e.g. Brand Launch 2025">
            </div>
            <div class="col-md-1">
              <label class="form-label fw-semibold">Currency</label>
              <select name="currency" id="currency" class="form-select">
                <?php foreach ($currencies as $c): ?>
                  <option value="<?= $c ?>" <?= old('currency', $proposal['currency']) === $c ? 'selected' : '' ?>><?= $c ?></option>
                <?php endforeach ?>
              </select>
            </div>
            <div class="col-md-2">
              ...
              <select name="multiplier" id="multiplier" class="form-select" onchange="recalcAll()">
                ...
                <?php foreach ($multipliers as $m): ?>
                  <option value="<?= $m ?>" <?= old('multiplier', $proposal['multiplier']) == (string)$m ? 'selected' : '' ?>><?= number_format($m, 1) ?>x</option>
                <?php endforeach ?>
              </select>
            </div>
            <div class="col-12">
              ...
              <textarea name="notes" class="form-control" rows="2"
                        placeholder="Any additional context for this proposal..."><?= old('notes', e($proposal['notes'] ?? '')) ?></textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="d-flex gap-2">
        ...
        <a href="<?= url("/proposals/{$proposal['id']}") ?>" class="btn btn-outline-secondary">Cancel</a>
      </div>
    </form>
  </div>
</div>
...
const EXISTING_ITEMS       = <?= json_encode($existingItems, JSON_HEX_TAG) ?>;
...
if (EXISTING_ITEMS.length) {
  EXISTING_ITEMS.forEach(it => addRow(it.position_id, it.designation, it.monthly_salary, it.allocation));
} else {
  addRow();
}
</script>
